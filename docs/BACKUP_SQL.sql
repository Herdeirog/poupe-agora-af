-- =====================================================================
-- POUPE AGORA 4.0 - BACKUP SQL COMPLETO
-- =====================================================================
--
-- Este é o backup completo e autônomo do banco de dados.
-- Use este arquivo para recriar TODO o banco em um novo projeto Supabase
-- caso a migração padrão (DATABASE_SETUP.sql) falhe.
--
-- COMO USAR:
--   1. Crie um novo projeto no Supabase Dashboard
--   2. Vá em SQL Editor (https://supabase.com/dashboard/project/SEU_PROJECT/sql/new)
--   3. Cole TODO o conteúdo deste arquivo e execute
--   4. Após a execução, use a Edge Function seed-admin para criar o usuário admin
--
-- IMPORTANTE:
--   - Execute em um projeto Supabase LIMPO (sem tabelas existentes)
--   - Todas as cláusulas usam IF NOT EXISTS / ON CONFLICT para segurança
...
-- 3.9 Trigger para criar profile no signup
...
DO $$ BEGIN CREATE POLICY "Admins can view all profiles" ON public.profiles FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can update all profiles" ON public.profiles FOR UPDATE USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- user_roles ----
DO $$ BEGIN CREATE POLICY "Users can view own roles" ON public.user_roles FOR SELECT TO authenticated USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can manage all roles" ON public.user_roles FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- categories ----
DO $$ BEGIN CREATE POLICY "Users view system categories and own categories" ON public.categories FOR SELECT USING ((user_id IS NULL) OR (user_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users manage own categories" ON public.categories FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins manage all categories" ON public.categories FOR ALL USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- transactions ----
DO $$ BEGIN CREATE POLICY "Users manage own transactions" ON public.transactions FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all transactions" ON public.transactions FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- goals ----
DO $$ BEGIN CREATE POLICY "Users manage own goals" ON public.goals FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all goals" ON public.goals FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- budgets ----
DO $$ BEGIN CREATE POLICY "Users manage own budgets" ON public.budgets FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all budgets" ON public.budgets FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- reminders ----
DO $$ BEGIN CREATE POLICY "Users manage own reminders" ON public.reminders FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all reminders" ON public.reminders FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- financial_commitments ----
DO $$ BEGIN CREATE POLICY "Users manage own commitments" ON public.financial_commitments FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all commitments" ON public.financial_commitments FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- recurring_payments ----
DO $$ BEGIN CREATE POLICY "Users manage own payments" ON public.recurring_payments FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all payments" ON public.recurring_payments FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- commitment_reminders ----
DO $$ BEGIN CREATE POLICY "Users can view their own commitment reminders" ON public.commitment_reminders FOR SELECT USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can create their own commitment reminders" ON public.commitment_reminders FOR INSERT WITH CHECK (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can update their own commitment reminders" ON public.commitment_reminders FOR UPDATE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can delete their own commitment reminders" ON public.commitment_reminders FOR DELETE USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can view all commitment reminders" ON public.commitment_reminders FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- subscriptions ----
DO $$ BEGIN CREATE POLICY "Users view own subscription" ON public.subscriptions FOR SELECT USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins manage subscriptions" ON public.subscriptions FOR ALL USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- subscription_payments ----
DO $$ BEGIN CREATE POLICY "Admins can view all payments" ON public.subscription_payments FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can insert payments" ON public.subscription_payments FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can update payments" ON public.subscription_payments FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can delete payments" ON public.subscription_payments FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- plans ----
DO $$ BEGIN CREATE POLICY "Anyone can view active plans" ON public.plans FOR SELECT USING (active = true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can manage plans" ON public.plans FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- admin_notifications ----
DO $$ BEGIN CREATE POLICY "Admins can view all notifications" ON public.admin_notifications FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can insert notifications" ON public.admin_notifications FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can update notifications" ON public.admin_notifications FOR UPDATE USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can delete notifications" ON public.admin_notifications FOR DELETE USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- admin_action_history ----
DO $$ BEGIN CREATE POLICY "Admins can view action history" ON public.admin_action_history FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can insert action history" ON public.admin_action_history FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- family_plans ----
DO $$ BEGIN CREATE POLICY "Admins can manage all family plans" ON public.family_plans FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can view own family plan" ON public.family_plans FOR SELECT USING (admin_user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can update own family plan" ON public.family_plans FOR UPDATE USING (admin_user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can create own family plan" ON public.family_plans FOR INSERT WITH CHECK (admin_user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- family_members ----
DO $$ BEGIN CREATE POLICY "Admins can manage all family members" ON public.family_members FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Plan admins can manage their members" ON public.family_members FOR ALL USING (EXISTS (SELECT 1 FROM public.family_plans WHERE id = family_plan_id AND admin_user_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Members can view their own membership" ON public.family_members FOR SELECT USING (user_id = auth.uid() OR email = (SELECT email FROM public.profiles WHERE id = auth.uid())); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- family_action_history ----
DO $$ BEGIN CREATE POLICY "Admins can view all family action history" ON public.family_action_history FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Plan admins can view their history" ON public.family_action_history FOR SELECT USING (EXISTS (SELECT 1 FROM public.family_plans WHERE id = family_plan_id AND admin_user_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Plan admins can insert history" ON public.family_action_history FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.family_plans WHERE id = family_plan_id AND admin_user_id = auth.uid()) OR has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- family_notifications ----
DO $$ BEGIN CREATE POLICY "Admins can manage all family notifications" ON public.family_notifications FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Plan admins can view their notifications" ON public.family_notifications FOR SELECT USING (EXISTS (SELECT 1 FROM public.family_plans WHERE id = family_plan_id AND admin_user_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Plan admins can insert notifications" ON public.family_notifications FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.family_plans WHERE id = family_plan_id AND admin_user_id = auth.uid()) OR has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Users can view notifications sent to them" ON public.family_notifications FOR SELECT USING (target_user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- user_reminders ----
DO $$ BEGIN CREATE POLICY "Users manage own reminders" ON public.user_reminders FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- goal_weeks ----
DO $$ BEGIN CREATE POLICY "Users manage own goal weeks" ON public.goal_weeks FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- goal_reminders ----
DO $$ BEGIN CREATE POLICY "Users manage own goal reminders" ON public.goal_reminders FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- global_settings ----
DO $$ BEGIN CREATE POLICY "Anyone can view global settings" ON public.global_settings FOR SELECT USING (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can manage global settings" ON public.global_settings FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- currency_rates ----
DO $$ BEGIN CREATE POLICY "Anyone can view currency rates" ON public.currency_rates FOR SELECT USING (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can manage currency rates" ON public.currency_rates FOR ALL USING (has_role(auth.uid(), 'admin'::app_role)); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- agents ----
DO $$ BEGIN CREATE POLICY "Admins manage agents" ON public.agents FOR ALL USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Public read active agents" ON public.agents FOR SELECT USING (active = true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- integration_evolution ----
DO $$ BEGIN CREATE POLICY "Admins manage evolution settings" ON public.integration_evolution FOR ALL USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- conversation_buffer ----
DO $$ BEGIN CREATE POLICY "Admins view buffer" ON public.conversation_buffer FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- agent_runs ----
DO $$ BEGIN CREATE POLICY "Admins view runs" ON public.agent_runs FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- inbound_messages ----
DO $$ BEGIN CREATE POLICY "Admins can view inbound_messages" ON public.inbound_messages FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- processing_locks ----
DO $$ BEGIN CREATE POLICY "Admins can view processing_locks" ON public.processing_locks FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- message_queue ----
DO $$ BEGIN CREATE POLICY "Admins can view message_queue" ON public.message_queue FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- encrypted_secrets ----
DO $$ BEGIN CREATE POLICY "Admins can insert secrets" ON public.encrypted_secrets FOR INSERT TO authenticated WITH CHECK (public.is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins can update secrets" ON public.encrypted_secrets FOR UPDATE TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- agenda_events ----
DO $$ BEGIN CREATE POLICY "Users manage own agenda events" ON public.agenda_events FOR ALL USING (user_id = auth.uid()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all agenda events" ON public.agenda_events FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ---- agenda_recurrences ----
DO $$ BEGIN CREATE POLICY "Users manage own recurrences" ON public.agenda_recurrences FOR ALL USING (auth.uid() = user_id); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE POLICY "Admins view all recurrences" ON public.agenda_recurrences FOR SELECT USING (is_admin()); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DROP TRIGGER IF EXISTS update_integration_evolution_updated_at ON public.integration_evolution;
...
DROP TRIGGER IF EXISTS update_encrypted_secrets_updated_at ON public.encrypted_secrets;
...
DROP TRIGGER IF EXISTS update_currency_rates_updated_at ON public.currency_rates;
...
DROP TRIGGER IF EXISTS update_financial_commitments_updated_at ON public.financial_commitments;
CREATE TRIGGER update_financial_commitments_updated_at BEFORE UPDATE ON public.financial_commitments
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_commitment_reminders_updated_at ON public.commitment_reminders;
...
DROP TRIGGER IF EXISTS update_family_plans_updated_at ON public.family_plans;
...
-- Tabelas de sistema (apenas leitura para admin via RLS)
...
DO $$ BEGIN CREATE POLICY "Admins can upload branding assets" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'branding'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- 12.4 Configuração Evolution API vazia
...
--
-- 2. Criar usuário admin via Edge Function seed-admin:
...
--
-- 3. Configurar secrets no Supabase Dashboard:
